...
namespace RuntimeNodeEditor
{
    public class NodeGraph : MonoBehaviour {
        ...
        public void Create(string prefabPath, Vector2 pos)
        {
            ...
            node.Init(_signalSystem, _signalSystem, pos, NewId(), prefabPath,this);
            node.Setup();
            nodes.Add(node);
        ...
        public void ClearConnectionsWithID(string socketID)
        {
            
            connections.Where(conn => conn.output.socketId == socketID || conn.input.socketId == socketID)
                .ToList()
                .ForEach(conn => Disconnect(conn));
        }
        ...
        private void LoadNode(NodeData data)
        {
            ...
            node.Init(_signalSystem, _signalSystem, pos, data.id, data.path,this);
            node.Setup();
            nodes.Add(node);
