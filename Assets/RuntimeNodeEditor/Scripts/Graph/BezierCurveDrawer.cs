...
namespace RuntimeNodeEditor
{
    public class BezierCurveDrawer : MonoBehaviour
    {
        ...
        /* From ChatGPT ...
         *  You nailed it: you’re tearing down a socket and Unity is trying to disconnect the same line twice, so BezierCurveDrawer.Remove(connId) is called a second time and throws because the curve isn’t in its dictionary anymore.
           What’s happening
           You click “clear connection” → NodeGraph.Disconnect(conn)
           Removes from drawer (OK)
           Removes from connections (OK)
           Fires OnDisconnectEvent → your ConcatNode.OnDisconnected(...) runs
           In OnDisconnected you call ForceLayout() → decides to remove the now-empty middle socket → RemoveInputSocket(si)
           RemoveInputSocket → DeRegister(si); inside Node.DeRegister it calls Graph.ClearConnectionsWithID(si.socketId) → tries to Disconnect again → drawer.Remove(connId) a second time ⇒ KeyNotFound.
